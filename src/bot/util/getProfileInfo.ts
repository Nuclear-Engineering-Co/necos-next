...
export default async (bot: Bot, username: string): Promise<[PlayerInfo, number]> => {
    const userId = bot.userIdCache[username] || await getIdFromUsername(username).catch(error => {
