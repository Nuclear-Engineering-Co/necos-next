...
export default class VerifyUpdateCommand extends BaseCommand {
  name = "update";
  description = "Allows users to obtain roles and have their nickname set to their Roblox username.";
  ...
  onCommand = async (interaction: ChatInputCommandInteraction<"cached">) => {
    ...
    const user = await this.database<User>("users").select("*")
    .where("discord_id", member.id)
    .first();

    const roleBinds = await this.database<VerificationRoleBind>("verification_binds")
    .select("*")
    .where({
      guild_id: guild.id,
    });
      ...
      return await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription("No userdata was found for your Discord account. Please ensure you are verified. (\`/verify link\`)")
            .setColor(Colors.Red)
            .setTimestamp()
            .setFooter({
    ...
    if (roleBinds.length == 0) {
      return await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setTitle(`${this.bot.environment.APP_NAME} Verification`)
            .setDescription("No bind data was found for this guild. Please contact a guild administrator to resolve this.")
            .setColor(Colors.Red)
            .setTimestamp()
            .setFooter({
    ...
    for (const roleBind of roleBinds) {
      ...
      switch (roleData.type) {
        case "group":
          ...
          if (!groupId || !rankNumber) {
            errors.push(`Role bind ${roleBind.id} is missing a required property for bind type GROUP. Please contact a guild administrator.`)
            break;
          }
          ...
          try {
            rankInGroup = rankCache[groupId] || await getRankInGroup(groupId, parseInt(user.user_id));
            rankCache[groupId] = rankInGroup;
          } catch (error) {
            errors.push(`An error occurred whilst fetching ${user.username}'s rank in group ${groupId}. ${error}`)
            break;
          }
      ...
      if (role && !shouldHaveRole) {
        try {
          ...
          errors.push(`An error occurred whilst removing role <@&${role.id}> from <@${member.id}>. ${error}`)
        }
      } else if (!role && shouldHaveRole) {
        role = await guild.roles.resolve(roleBind.role_id);

        if (!role) {
          errors.push(`An error occurred whilst fetching role <@&${roleBind.role_id}> from the guild. Please try again later.`)
          continue;
        }
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setTitle(`${this.bot.environment.APP_NAME} Verification`)
          .setDescription(`Update for <@${member.id}> complete. ${errors.length > 0 ? "Errors:\n" + errors.join("\n") : ""}`)
